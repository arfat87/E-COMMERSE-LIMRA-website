...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === "POST") {
      ...
      if (numTable) {
        try {
          ...
          if (existing && existing.length > 0) {
            computedRound = Math.max(computedRound, existing.length + 1);
          }
        } catch (e) {}
      }

      let rawNotes = notes ? notes.trim() : "";
      if (numTable && !/\[ROUND:\s*\d+\]/i.test(rawNotes)) {
      ...
      try {
        const notifTitle = tableNumber
          ? computedRound > 1
            ? `🚨 Table ${tableNumber} - Round ${computedRound}!`
    ...
    if (req.method === "PATCH" || req.method === "PUT") {
      ...
      const { data: updated, error: updErr } = await q.select("*, items:order_items(*)");
